"""Single-file SAP2000 import tool variant backed by comtypes."""
...
class Sap2000Session:
    ...
    def _query_helper_interface(self, helper: Any) -> Any:
        try:
            from comtypes.gen import SAP2000v1

            return helper.QueryInterface(SAP2000v1.cHelper)
        except Exception:
            return helper

    def _create_sap_object(self, client: Any) -> Any:
        errors: list[str] = []
        ...
        try:
            return client.CreateObject(SAP_PROGID)
        except Exception as exc:
            errors.append(f"CreateObject({SAP_PROGID!r}) failed: {exc}")
    ...
    def __enter__(self) -> "Sap2000Session":
        try:
            ...
            raise RuntimeError(
                "comtypes is required to connect to SAP2000. Install comtypes in the runtime environment."
            ) from exc

        self._comtypes = comtypes
        comtypes.CoInitialize()
        try:
            helper = client.CreateObject("SAP2000v1.Helper")
            self.helper = self._query_helper_interface(helper)
            if self.attach_to_instance:
                try:
                    if hasattr(self.helper, "GetObject"):
                        self.SapObject = self.helper.GetObject(SAP_PROGID)
                    else:
                        self.SapObject = client.GetActiveObject(SAP_PROGID)
                except Exception:
                    self.SapObject = None
            ...
            if self.SapObject is None:
                ...
                self.SapObject = self._create_sap_object(client)
...
def run_sap2000_worker_analysis(
    ...
    *,
    ...
    script_path = Path(__file__).parent / "run_sap2000_model_comptyes.py"
...
async def build_sap_model_from_analytical_json_comptypes_func(ctx: Any, args: str) -> str:
    del ctx
    ...
    try:
        ...
        return (
            f"Imported analytical model into SAP2000 with comtypes. {source_note}. {support_note} "
        ...
        return f"Error building SAP2000 model from analytical JSON with comtypes: {error_msg}"
    except Exception as exc:
        logger.exception("Unexpected error in build_sap_model_from_analytical_json_comptypes_func")
        return f"Unexpected error: {type(exc).__name__}: {exc}"


def build_sap_model_from_analytical_json_comptypes_tool() -> Any:
    ...
    return FunctionTool(
        ...
        on_invoke_tool=build_sap_model_from_analytical_json_comptypes_func,
    )
